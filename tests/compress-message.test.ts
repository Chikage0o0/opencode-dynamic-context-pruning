...
test("compress message tool uses selection-only prompt when pruningModel is configured", () => {
    ...
    const tool = createCompressMessageTool({
        ...
        prompts: {
            reload() {},
            getRuntimePrompts() {
                return { compressMessage: "THE SUMMARY conflicting instruction", compressRange: "" }
            },
        },
    } as any)
...
test("compress message delegates summary generation when pruningModel is configured", async () => {
    const sessionID = `ses_message_pruning_model_${Date.now()}`
    const rawMessages = buildMessages(sessionID)
    const state = createSessionState()
    const logger = new Logger(false)
    const config = buildConfig()
    ...
    const tool = createCompressMessageTool({
        ...
        pruningSummaryGenerator: {
            async generateSummary(request: any) {
                ...
                return "Message summary generated by the configured pruning model."
            },
        },
    } as any)
    ...
    await tool.execute(
        ...
        } as any,
        {
            ...
            metadata: () => {},
            sessionID,
            messageID: "msg-compress-message-pruning-model",
        },
    )

    assert.equal(summaryRequests.length, 1)
    assert.equal(summaryRequests[0].mode, "message")
    ...
    assert.equal(block.topic, "Code path note")
    assert.match(block.summary, /Message summary generated by the configured pruning model\./)
})

test("compress message falls back to batch topic when pruningModel entry topic is omitted", async () => {
    const sessionID = `ses_message_pruning_batch_topic_${Date.now()}`
    const rawMessages = buildMessages(sessionID)
    const state = createSessionState()
    const logger = new Logger(false)
    const config = buildConfig()
    ...
    const tool = createCompressMessageTool({
        ...
        pruningSummaryGenerator: {
            async generateSummary(request: any) {
                summaryRequests.push(request)
                return "Batch topic fallback summary."
            },
        },
    } as any)
    ...
    await tool.execute(
        ...
        } as any,
        {
            ...
            metadata: () => {},
            sessionID,
            messageID: "msg-compress-message-pruning-batch-topic",
        },
    )

    assert.equal(summaryRequests[0].topic, "Delegated Batch Topic")
    const block = Array.from(state.prune.messages.blocksById.values())[0]
    assert.equal(block.topic, "Delegated Batch Topic")
    assert.match(block.summary, /Batch topic fallback summary\./)
})
