...
test("compress range tool uses selection-only prompt when pruningModel is configured", () => {
    const config = buildConfig()
    config.pruningModel = "openai/gpt-4o-mini"
    const tool = createCompressRangeTool({
        ...
        prompts: {
            reload() {},
            getRuntimePrompts() {
                return { compressRange: "THE SUMMARY conflicting instruction", compressMessage: "" }
            },
        },
    } as any)

    assert.match(tool.description, /SUMMARY DELEGATION/)
    assert.match(tool.description, /Do not write, draft, or include replacement summaries/)
    assert.doesNotMatch(tool.description, /THE SUMMARY conflicting instruction/)
})
...
test("compress range delegates summary generation when pruningModel is configured", async () => {
    const sessionID = `ses_range_pruning_model_${Date.now()}`
    const rawMessages = buildMessages(sessionID)
    const state = createSessionState()
    const logger = new Logger(false)
    const config = buildConfig()
    ...
    const tool = createCompressRangeTool({
        ...
        pruningSummaryGenerator: {
            async generateSummary(request: any) {
                ...
                return "Summary generated by the configured pruning model."
            },
        },
    } as any)
    ...
    await tool.execute(
        ...
        } as any,
        {
            ...
            metadata: () => {},
            sessionID,
            messageID: "msg-compress-range-pruning-model",
        },
    )

    assert.equal(summaryRequests.length, 1)
    assert.equal(summaryRequests[0].mode, "range")
    assert.equal(summaryRequests[0].model, "openai/gpt-4o-mini")
    assert.deepEqual(
        summaryRequests[0].messages.map((message: any) => message.ref),
        ["m0001", "m0002"],
    )

    const block = Array.from(state.prune.messages.blocksById.values())[0]
    assert.match(block.summary, /Summary generated by the configured pruning model\./)
})

test("compress range with pruningModel consumes existing blocks without duplicating their summaries", async () => {
    const sessionID = `ses_range_pruning_existing_block_${Date.now()}`
    const rawMessages = buildMessages(sessionID)
    const firstUserPart = rawMessages[0]?.parts[0]
    if (firstUserPart?.type === "text") {
        firstUserPart.text =
            "Investigate the issue. <protect>Preserve consumed block protected note.</protect>"
    }
    const state = createSessionState()
    const logger = new Logger(false)
    const client = {
    ...
    const initialTool = createCompressRangeTool({
        client,
        state,
        logger,
            ...
            content: [
                {
                    startId: "m0001",
                    endId: "m0002",
                    summary: "First compressed summary should not be duplicated later.",
                },
            ],
        },
        {
            ...
            metadata: () => {},
            sessionID,
            messageID: "msg-compress-initial-block",
        },
    )

    const pruningConfig = buildConfig()
    pruningConfig.pruningModel = "openai/gpt-4o-mini"
    pruningConfig.compress.protectTags = true
    const pruningTool = createCompressRangeTool({
        client,
        state,
        logger,
        ...
        pruningSummaryGenerator: {
            async generateSummary() {
                return "Delegated pruning summary over the raw selected content."
            },
        },
    } as any)

    await pruningTool.execute(
        {
            topic: "Merged block",
            content: [
                {
        ...
        } as any,
        {
            ...
            metadata: () => {},
            sessionID,
            messageID: "msg-compress-pruning-existing-block",
        },
    )

    const initialBlock = state.prune.messages.blocksById.get(1)
    const mergedBlock = state.prune.messages.blocksById.get(2)
    assert.equal(initialBlock?.active, false)
    assert.deepEqual(mergedBlock?.consumedBlockIds, [1])
    assert.match(
        mergedBlock?.summary || "",
        /Delegated pruning summary over the raw selected content\./,
    )
    assert.match(mergedBlock?.summary || "", /Preserve consumed block protected note\./)
    assert.doesNotMatch(
        mergedBlock?.summary || "",
        /First compressed summary should not be duplicated later\./,
    )
})
