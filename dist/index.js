...
function collectTurnNudgeAnchors2(state, messages, hardMode) {
  const turnNudgeAnchors = /* @__PURE__ */ new Set();
  const targetRole = hardMode ? "user" : "assistant";
  for (const message of messages) {
...
function applyAnchoredNudges(state, config, messages, prompts, compressionPriorities, hardMode = false) {
  const turnNudgeAnchors = collectTurnNudgeAnchors2(state, messages, hardMode);
  if (config.compress.mode === "message") {
